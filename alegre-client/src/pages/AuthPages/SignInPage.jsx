...
const SignInPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const destination =
        from && from !== '/auth/signin' && from !== '/auth/signup'
          ? from
          : '/'; 
          
      // Defer navigation to the next event loop tick.
      // This stops React Router from canceling the redirect mid-render.
      setTimeout(() => {
      ...
      const msg =
        err.response?.data?.message || 'Sign in failed. Is the API server running?';
      alert(msg);
    } finally {
      setSubmitting(false);
    }
